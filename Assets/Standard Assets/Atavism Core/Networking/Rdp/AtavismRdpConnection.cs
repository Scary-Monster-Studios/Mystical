...
namespace Atavism
{
    ...
    public class ConnectionCallback
    {
        ...
        public object state;
        public AtavismRdpPacket packet;
    }
    ...
    public class AtavismRdpConnection
    {
        ...
        AtavismRdpConnectionManager connManager;
        ...
        Dictionary<int, DateTime> retransmissionTimer = new Dictionary<int, DateTime>();
        Dictionary<int, AtavismRdpPacket> unacknowledgedPackets = new Dictionary<int, AtavismRdpPacket>();
        SortedList<int, AtavismRdpPacket> outOfOrderPackets = new SortedList<int, AtavismRdpPacket>();
        List<AtavismRdpPacket> availablePackets = new List<AtavismRdpPacket>();
        SortedList<int, AtavismRdpPacket> availableOutOfOrderPackets = new SortedList<int, AtavismRdpPacket>();


        /// <summary>
        ///   Constructor - should only be called by the connection manager object
        /// </summary>
        public AtavismRdpConnection(AtavismRdpConnectionManager connManager,bool passive, UdpClient udpConn, IPEndPoint remoteEP, int rcvMax, int rbufMax, bool sequenced)
        {
        ...
        private int[] AbridgedEakArray
        {
            get
            {
                ...
                if (eakArray.Length > AtavismRdpPacket.MaxEaks)
                {
                    int[] abridgedArray = new int[AtavismRdpPacket.MaxEaks];
        ...
        private void HandleDataPacket(AtavismRdpPacket packet)
        {
            ...
            foreach (int segSeq in sortedSequence)
            {
                if (segSeq == rcvCur + 1)
                {
                    AtavismRdpPacket currentPacket = outOfOrderPackets[segSeq];
        ...
        private void InternalOpen(bool passiveOpen)
        {
            ...
            if (!passiveOpen)
            {
                ...
                AtavismRdpPacket packet = new AtavismRdpPacket(0, AtavismRdpPacket.OpenLength);
        ...
        /// </summary>
        /// <param name="packet"></param>
        private void SendPacket(AtavismRdpPacket packet)
        {
            ...
            if (packet.HasData)
            {
                ...
                if (packet.PacketLength + IpHeaderLength + UdpHeaderLength > sbufMax)
                    throw new RdpFragmentationException("packet size "+packet.DataLength+" is too large for connection with max of "+sbufMax, packet.DataLength, sbufMax);
                ...
                //UnityEngine.Debug.Log("SendPacket: " + now + " - packet " + packet);
            }
            //if (packet.Eak)
            //	UnityEngine.Debug.Log("sending packet with eak set: " + packet);
        ...
        public void Send(byte[] data)
        {
            ...
            AtavismRdpPacket packet = new AtavismRdpPacket(data.Length);
        ...
        /// </summary>
        /// <param name="packet"></param>
        public void Send(AtavismRdpPacket packet)
        {
            if (!valid)
                throw new Exception("Called Send on closed connection");
            try
        ...
        /// </summary>
        /// <param name="packet"></param>
        private void InternalSend(AtavismRdpPacket packet)
        {
            switch (state)
            {
                case ConnectionState.Open:
                //    UnityEngine.Debug.LogWarning("InternalSend: sndNxt="+ sndNxt+ " sndUna="+ sndUna+ " sndMax="+ sndMax);
                ...
                case ConnectionState.CloseWait:
                   // AtavismLogger.LogError("Got state: " + state);
                ...
                case ConnectionState.Open:
                    ...
                    try
                    {
                        ...
                        while (true) {
                            ...
                            if (!this.State.Equals(ConnectionState.Open))
                            {
                                throw new Exception("Error - connection not open");
                            }
        ...
        {
         //   UnityEngine.Debug.LogError("AtavismRdpConnection.InternalClose Start");
                ...
                case ConnectionState.Open:
                    {
                        ...
                        AtavismRdpPacket packet = new AtavismRdpPacket(0);
                        packet.SeqNumber = sndNxt;
                        packet.Rst = true;
                        SendPacket(packet);
                    }
                    ...
                    AtavismLogger.LogInfoMessage("InternalClose() called with stackTrace: " + t.ToString());
                ...
                case ConnectionState.SynSent:
                    {
                        ...
                        AtavismLogger.LogInfoMessage("InternalClose() called with stackTrace: " + t1.ToString());
                    }
                    State = ConnectionState.Closed;
                    break;
                case ConnectionState.CloseWait:
         ...
         //   UnityEngine.Debug.LogError("AtavismRdpConnection.InternalClose End");
        ...
        public void OnSegmentArrival(AtavismRdpPacket inPacket, IPEndPoint remoteEP)
        {
            try
            {
                Monitor.Enter(this);
        ...
        private void InternalOnSegmentArrival(AtavismRdpPacket inPacket, IPEndPoint remoteEP)
        {
            DateTime now = DateTime.Now;
        //    UnityEngine.Debug.Log("OnSegmentArrival: " + now + " - packet " + inPacket+ " \nstate="+ state);
            ...
            switch (state)
            {
                ...
                case ConnectionState.CloseWait:
                    ...
                    else if (inPacket.Ack || inPacket.Nul)
                    {
                        ...
                        SendPacket(packet);
                        System.Diagnostics.StackTrace t = new System.Diagnostics.StackTrace();
                        AtavismLogger.LogInfoMessage("InternalOnSegmentArrival() RST 620 send called with stackTrace: " + t.ToString());
                    }
                    else
                    {
                        ...
                        SendPacket(packet);
                        System.Diagnostics.StackTrace t = new System.Diagnostics.StackTrace();
                        AtavismLogger.LogInfoMessage("InternalOnSegmentArrival() RST 632 send called with stackTrace: " + t.ToString());
                ...
                case ConnectionState.Listen:
                    ...
                    if (inPacket.Ack || inPacket.Nul)
                    {
                        ...
                        SendPacket(packet);
                        System.Diagnostics.StackTrace t = new System.Diagnostics.StackTrace();
                        AtavismLogger.LogInfoMessage("InternalOnSegmentArrival() RST 647 send called with stackTrace: " + t.ToString());
                        return;
                    }
                    if (inPacket.Syn)
                    {
                ...
                case ConnectionState.SynSent:
                    ...
                    if (inPacket.Syn)
                    {
                        ...
                        if (inPacket.Ack)
                        {
                            ...
                            //Logger.LogDebugMessage("OnSegmentArrival: setting state to open");
                    ...
                    if (inPacket.Ack)
                    {
                        if (!inPacket.Rst && inPacket.AckNumber != sndIss)
                        {
                            ...
                            State = ConnectionState.Closed;
                            System.Diagnostics.StackTrace t = new System.Diagnostics.StackTrace();
                            AtavismLogger.LogInfoMessage("InternalOnSegmentArrival() RST 729 send called with stackTrace: " + t.ToString());
                ...
                case ConnectionState.SynRcvd:
                    ...
                    if (inPacket.Syn)
                    {
                        ...
                        State = ConnectionState.Closed;
                        System.Diagnostics.StackTrace t = new System.Diagnostics.StackTrace();
                        AtavismLogger.LogInfoMessage("InternalOnSegmentArrival() RST 773 send called with stackTrace: " + t.ToString());
                    ...
                    if (inPacket.Eak)
                    {
                        ...
                        SendPacket(packet);
                        System.Diagnostics.StackTrace t = new System.Diagnostics.StackTrace();
                        AtavismLogger.LogInfoMessage("InternalOnSegmentArrival() RST 785 send called with stackTrace: " + t.ToString());
                        return;
                    }
                    if (inPacket.Ack)
                    {
                        ...
                        else
                        {
                            ...
                            SendPacket(packet);
                            System.Diagnostics.StackTrace t = new System.Diagnostics.StackTrace();
                            AtavismLogger.LogInfoMessage("InternalOnSegmentArrival() RST 800 send called with stackTrace: " + t.ToString());
                            return;
                        }
                    }
                    else
                    ...
                    if (inPacket.HasData || inPacket.Nul)
                    {
                        ...
                        AtavismRdpPacket packet = new AtavismRdpPacket(0, eakArray.Length * 4);
                        packet.SeqNumber = sndNxt;
                        packet.AckNumber = rcvCur;
                        packet.Ack = true;
                ...
                case ConnectionState.Open:
                    ...
                    {
                        State = ConnectionState.CloseWait;
                       // AtavismLogger.LogError("Connection Reset");
                        return;
                    }
                    ...
                    if (inPacket.Syn)
                    {
                        ...
                        State = ConnectionState.Closed;
                        System.Diagnostics.StackTrace t = new System.Diagnostics.StackTrace();
                        AtavismLogger.LogInfoMessage("InternalOnSegmentArrival() RST 864 send called with stackTrace: " + t.ToString());
                        //log.Warn("Connection Reset (by SYN)");
                        // TODO: deallocate connection
                        return;
                    }
