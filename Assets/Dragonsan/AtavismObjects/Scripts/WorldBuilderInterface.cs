...
namespace Atavism
{
    ...
    public class WorldBuilderInterface : MonoBehaviour
    {
        ...
        void Update()
        {
            if (AtavismCursor.Instance == null || AtavismCursor.Instance.IsMouseOverUI() || WorldBuilder.Instance.ActiveClaim==null)
                return;
            ...
            if (GetBuildingState() == WorldBuildingState.SelectItem)
            {
                ...
                if (Physics.Raycast(ray, out hit, Mathf.Infinity, objectSelectLayers))
                {
                   ...
                   // Debug.Log("hit go "+hit.transform.gameObject);
                  //  Debug.Log("hit co "+hit.transform.gameObject.GetComponent<ClaimObject>());
            ...
            if ((GetBuildingState() == WorldBuildingState.PlaceItem || GetBuildingState() == WorldBuildingState.MoveItem) && currentReticle != null)
            {
                ...
                if (Input.GetMouseButtonDown(0) && !AtavismUiSystem.IsMouseOverFrame())
                {
                    ...
                    else if (GetBuildingState() == WorldBuildingState.MoveItem)
                    {
                        ...
                        itemBeingPlaced = null;
                        WorldBuilder.Instance.SelectedObject.ObjectPlaced = true;
        ...
        public void OnEvent(AtavismEventData eData)
        {
            ...
            else if (eData.eventType == "START_BUILD_CLAIM_OBJECT")
            {
                ...
                int activeClaimType = WorldBuilder.Instance.ActiveClaim.claimType;
                if (activeClaimType > 0 && buildTemplate.validClaimTypes.Count > 0 )
                {
                    if (!buildTemplate.validClaimTypes.Contains(activeClaimType))
                    {
                        string[] args = new string[1];
#if AT_I2LOC_PRESET
