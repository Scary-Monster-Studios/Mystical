       ...
       //  Dictionary<string, DsWindowsSettings> windowsSettings;
            ...
            try
            {
                ...
                for (int i = 0; i < num; i++)
                {
                 ...
                 //   Debug.LogError("HandleRacePrefabData raceId="+raceId);
                    ...
                    race.iconData = (string) props["raceIcon" + i];
                    Texture2D tex = new Texture2D(2, 2);
                    bool wyn = tex.LoadImage(System.Convert.FromBase64String(race.iconData));
                    race.icon = Sprite.Create(tex, new Rect(0.0f, 0.0f, tex.width, tex.height), new Vector2(0.5f, 0.5f), 100.0f);
                  ...
                  //  Debug.LogError("HandleRacePrefabData classId="+classId);
                    ...
                    c.iconData = (string) props["classIcon" + i];
                    tex = new Texture2D(2, 2);
                    wyn = tex.LoadImage(System.Convert.FromBase64String(c.iconData));
                    c.icon = Sprite.Create(tex, new Rect(0.0f, 0.0f, tex.width, tex.height), new Vector2(0.5f, 0.5f), 100.0f);
                 ...
                 //   Debug.LogError("HandleRacePrefabData gnum=" + gnum);
                    ...
                    for (int k = 0; k < gnum; k++)
                    {
                       ...
                       // Debug.LogError("HandleRacePrefabData genderId="+genderId+" c.genderList.count="+c.genderList.Count);
                        ...
                        gender.iconData = (string) props["genIcon" + i + "_" + k];
                        tex = new Texture2D(2, 2);
                        wyn = tex.LoadImage(System.Convert.FromBase64String(gender.iconData));
                        gender.icon = Sprite.Create(tex, new Rect(0.0f, 0.0f, tex.width, tex.height), new Vector2(0.5f, 0.5f), 100.0f);
        ...
        void Save()
        {
            ...
            else
            {
                ...
                PlayerPrefs.SetInt("kskill", (int)settings.keySettings.skills);
...
//                settings.questListSelected = (PlayerPrefs.GetString("qs").Split(';').Select(n => Convert.ToInt32(n)).ToArray()).OfType<long>().ToList();
               ...
               .ToDictionary(s => (long)Convert.ToInt32(s[0]), s => (s[1].Split(';').Select(n => Convert.ToInt32(n)).ToArray()).OfType<long>().ToList());
                ...
                settings.keySettings.skills = (KeyCode)PlayerPrefs.GetInt("kskill");//, settings.keySettings.skiles.ToString());
