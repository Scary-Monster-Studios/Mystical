...
namespace Atavism
{
    ...
    public class AtavismMecanimMobController3D : MobController3D
    {
       ...
       // string weaponType;
      //  string weapon2Type;
        private  List<object>  weapontypes = new  List<object> ();
        [SerializeField]
        ...
        private AtavismMount _atavismMount;
        ...
        void Start()
        {
         ...
         //   {
                if (GetComponent<AtavismNode>() != null)
                {
                    ...
                    GetComponent<AtavismNode>().RegisterObjectPropertyChangeHandler("playerShop", HandlePlayerShop);
                    if (GetComponent<AtavismNode>().PropertyExists("playerShop"))
                    {
                        shop = (bool)GetComponent<AtavismNode>().GetProperty("playerShop");
                    }
                    
                    if (GetComponent<AtavismNode>().PropertyExists("weaponType"))
                    {
                         weapontypes = (List<object>)GetComponent<AtavismNode>().GetProperty("weaponType");
        ...
        void OnDestroy()
        {
            ...
            if (aNode != null)
            {
              ...
              //  aNode.RemoveObjectPropertyChangeHandler("weapon2Type", HandleWeapon2Type);
                ...
                aNode.RemoveObjectPropertyChangeHandler("playerShop", HandlePlayerShop);
        ...
        private float speedMount = 0f;
        void LateUpdate()
        {
            base.LateUpdate();
            Profiler.BeginSample("AMMC3D LateUpdate");
            ...
            if (_animator == null)
            {
                Debug.LogWarning("Not found Animator component on the this GameObject or on the child",gameObject);
                return;
            }
            ...
            //   if (_animator && !_animator.isInitialized)
            // return;
            // Debug.Log("Using animator for mob: " + name+" "+controller.velocity.magnitude+" "+controller);
            speed = controller.velocity.magnitude;
            if (mount != null)
            {
                    _atavismMount = mount.GetComponent<AtavismMount>();
                    speedMount = _atavismMount.speed;
            }
            ...
            else if (_animator)
            {
                ...
                else
                {
                    if (mount != null)
                    {
                        _animator.SetFloat("Speed", speedMount);
                    }
                    else
                    {
               ...
               // if (_animator.GetFloat("GroundDistance") != null)
                ...
                _animator.SetBool("playerShop", shop);
                _animator.SetBool("Combat", inCombat);
                //if (weaponType != null && weaponType != "" && weaponType != "Unarmed")
               // {
                    foreach (AnimatorControllerParameter param in _animator.parameters)
                    {
                        if (weapontypes.Contains(param.name))
        ...
        public override void StartMount(string mountPrefab)
        {
            base.StartMount(mountPrefab);

        }

         public override void StopMount()
        {
          //   Debug.LogError("StopMount AMMC3d" );
            if (_animator != null )
            {
                _animator.SetBool(mountAnim, false);
            }
        ...
        public void HandlePlayerShop(object sender, PropertyChangeEventArgs args)
        {
            shop = (bool)AtavismClient.Instance.WorldManager.GetObjectNode(oid).GetProperty("playerShop");
            if (_animator != null )
            {
             //   Debug.LogError("setting playerShop for animator :" + shop);
                _animator.SetBool("playerShop", shop);
            }
        }
        ...
        {
           /* if (_animator != null && weaponType != null && weaponType != "" && weaponType != "Unarmed")
            {
      ...
      /*  public void HandleWeapon2Type(object sender, PropertyChangeEventArgs args)
        ...
        public override void PlayAnimation(string animationName, float length)
        {
            ...
            if (_animator != null && overrideAnimationName != null && overrideAnimationName != "" && overrideAnimationName != animationName)
            {
